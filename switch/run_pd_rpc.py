...
try:
    ...
    print """
 ERR: Failed to load one or more of the following modules:
...
try:
    ...
    print " ERR: Failed to load Thrift modules."
    print "      Please, check your Thrift installation"
    sys.exit(1)
...
#        load_contrib(proto)
#except:
#    print "WARN: Scapy is not available. This is OK if you don't need it"#
...
try:
    ...
    print "WARN: readline module is not available. The tool will work,"
    print "      but command history wil not be saved"
...
try:
    ...
    try:
        ...
        print "WARN: jedi or rlcompleter modules are not available." 
        print "      The tool will work, but there will be no autocompletion"
...
def import_star(module_name):
    ...
    try:
        ...
        print " ERR: Cannot load module", module_name
        raise ImportError
    ...
    if not import_done:
        print "INFO: from %-20s import *" % (module_name)
...
class bfThrift( object ):
    ...
    def __init__( self, port, module, api_name,
        ...
        if port not in bprotocols.keys():
            ...
            print 'Aready connected to port {} using a different protocol'.format(
                    port)
        ...
        #
        if not import_done:
            print ("INFO: Use %-20s to access %s APIs" %
                   (var_name if var_name != None else api_name, api_name))
...
def open_session():
    ...
    if sess_hdl is None:
        ...
        print "INFO: Opened PD  API Session (sess_hdl): {}".format(
            sess_hdl)
    else:
        if show_trace:
            print "INFO: Continue PD  API Session (sess_hdl): {}".format(
                sess_hdl)

    if not args.no_mc:
        if mc_sess is None:
            mc_sess = mc.create_session()
            print "INFO: Opened MC  API Session (mc_sess) : {:#08x}".format(
                mc_sess)
        else:
            if show_trace:
                print "INFO: Continue MC  API Session (mc_sess) : {:#08x}".format(mc_sess)

    if TARGET == "asic" or TARGET == "asic-model":
        if pkt_sess is None:
            pkt.init()
            pkt_sess = pkt.client_init()
            print "INFO: Opened PKT API Session (pkt_sess): {}".format(pkt_sess)
        else:
            if show_trace:
                print "INFO: Continue PKT API Session (pkt_sess): {}".format(
                    pkt_sess)
    print
...
def close_session():
    try:
        if sess_hdl is not None:
            print "INFO: Closing session {}".format(sess_hdl)
            conn_mgr.client_cleanup()
        
        if mc_sess is not None:
            print "INFO: Closing MC API session {:#08x}".format(mc_sess)
            mc.destroy_session()
        
        if pkt_sess is not None:
            print "INFO: Closing Packet Manager Session {}".format(pkt_sess)
            pkt.client_cleanup()
            pkt.cleanup()
    except:
        pass
...
def import_common_apis():
    ...
    try:
        ...
        print "      Make sure your SDE is built for %s architecture running on %s target" % (ARCH, TARGET)
        sys.exit(1)
    ...
    try:
        ...
        print "WARN: Cannot load port_mapping module. "
        print "      Port mapping functions will not be available"
    ...
    # Fortunately, they do not really care and so EVERYONE (even tables) accept
    # p4_pd.counter_flags_t. So that's what we'll use. Yes, this is a hack
    ...
    if TARGET == "asic":
        ...
        try:
            ...
            print "WARN: Platform Manager APIs are not available:", e.message
            print "      Ignore this message if you are running against the"
            print "      asic-model or add '--target asic-model' to the "
            print "      command line to suppress this message"
...
def import_diag_apis():
    ...
    try:
        ...
        if show_trace:
            print "WARN: diagAPI is not available:", e.message
...
def import_switch_apis():
    ...
    try:
        ...
        if show_trace:
            print "WARN: switchAPI is not available:", e.message
    ...
    try:
        ...
        if show_trace:
            print "WARN: SAI is not available:", e.message
...
def import_all_apis():
    ...
    try:
        ...
        Here are common mistakes:
           ...
           -- IP address (%s) is not correct
              -- If you are trying to connect to a remote host, check
                 connectivity
           -- Thrift port (%d) is not correct
...
def reset_connection():
    ...
    for port in bprotocols:
        if show_trace:
            print "Resetting connection on port %d" % (port)
...
def handle_traceback():
    ...
    bf_exceptions = [p4_pd.InvalidCounterOperation,
                     ...
                     conn_mgr.InvalidConnMgrOperation,
    ...
    if  type in bf_exceptions:
        ...
        # use their own codes
        print "ERROR:", decode_error(repr(value)), "(%d)" % value.code
...
def setdev(device):
    ...
    if PROG == PREFIX:
        sys.ps1="PD(%s)[%d]>>> " % (PROG, dev)
    else:
        sys.ps1="PD(%s/%s)[%d]>>> " % (PROG, PREFIX, dev)
...
def check_env():
    ...
    try:
        ...
        print "ERROR: SDE environment variable is not set"
        quit()

    try:
        SDE_INSTALL= os.environ["SDE_INSTALL"]
    except KeyError:
        print "WARNING: SDE_INTALL environment variable is not set"
        print "         Assuming $SDE/install"
        SDE_INSTALL = "/".join([SDE, "install"])

    print "INFO: Using         SDE", SDE
    print "INFO: Using SDE_INSTALL", SDE_INSTALL

def parse_args():
    global PROG, PREFIX, TARGET, ARCH
    global args
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The name of the P4 program (P4_NAME) to interface with")
    parser.add_argument(
        "-P", "--prefix",
        type=str,
        help="""
    ...
    parser.add_argument(
        "-d", "--target-type",
        type=str.lower,
        choices=["asic", "bmv2", "asic-model", "hw"],
        #default="asic",
        help="""
        P4 Target:
    ...
    parser.add_argument(
        "-b", "--arch-type",
        type=str.lower,
        choices=["simple_switch", "tofino", "SimpleSwitch"],
        default="tofino",
        help="""
        P4 Architecture:
            tofino        -- Tofino architecture
            simple_switch -- Standard P4 architecture, defined in P4_14 spec
        """)
    parser.add_argument(
        "--thrift-ip",
        type=str,
    ...
    ARCH   = args.arch_type.lower()
    if ARCH == "simpleswitch":
        ARCH = "simple_switch"

    if TARGET == None:
        if ARCH != "simple_switch":
    ...
    print "INFO: Running on", TARGET, "in", ARCH, "architecture"
...
def set_paths(install_dir):
    ...
    if (TARGET == "asic" or
        ...
        if (ARCH == "simple_switch" or
            ARCH == "simpleswitch"):
            sys.path.append(os.path.join(SDE_PYPATH, "bm", "pdfixed"))
            sys.path.append(os.path.join(SDE_PYPATH, "bmpd"))
        elif ARCH == "tofino":
            sys.path.append(os.path.join(SDE_PYPATH, "tofinobm", "pdfixed"))
            sys.path.append(os.path.join(SDE_PYPATH, "tofinobmpd"))
        else:
    ...
    sys.path.insert(0, os.path.expanduser(
        os.path.join('~', '.pd_rpc', 'rc', PROG)))

def wait_for_switchd():
    if TARGET == "asic" or TARGET == "asic-model":
        os.system("python %s/p4testutils/bf_switchd_dev_status.py --host %s --port %s" % (SDE_PYPATH, args.thrift_ip, 7777))
...
def rc_load(c):
    if not args.no_rcload:
        ...
        rc_exec(c, rc_file)
        rc_file = os.path.expanduser(
            os.path.join("~", '.pd_rpc', '_rc_', PROG, '_rc_.py'))
        rc_exec(c, rc_file)
    else:
        print "INFO: Skipping loading rc files"
...
def run_shell(script):
    ...
    if readline_support:
        ...
        histfile = os.path.join(histdir, PROG)
        try:
...
def print_match_spec(match_spec):
    ...
    for f in filtered_field_list:
        print "    %-30s" % f, "=", myrepr(getattr(match_spec, f))

def print_action_desc(action_desc):
    print "Action:\n   ", action_desc.name
    action_data_attr = "%s_%s" % (PREFIX, action_desc.name)
    if hasattr(action_desc.data, action_data_attr):
        ...
        if hasattr(action_data, "__dict__"):
            ...
            for f in sorted(action_data.__dict__.keys()):
                print "    %-30s" % f[7:], "=", myrepr(getattr(action_data, f))
...
def print_entry(entry):
    ...
    if hasattr(entry, "has_mbr_hdl"):
        if getattr(entry, "has_mbr_hdl") == 1:
            print "Action Profile Member: 0x%08x (%d)" % (
                entry.action_mbr_hdl, entry.action_mbr_hdl)

    if hasattr(entry, "has_grp_hdl"):
        if getattr(entry, "has_grp_hdl") == 1:
            print "Action Profile Group: 0x%08x (%d)" % (
                entry.selector_grp_hdl, entry.selector_grp_hdl)
#
# Table dumping
#
...
than "count" entries.
    ...
    try:
        exec "ec = p4_pd.%s_get_entry_count(sess_hdl=sess, dev_tgt=tgt)" % table
    ...
    exec "fh = p4_pd.%s_get_first_entry_handle(sess_hdl=sess, dev_tgt=tgt)" % table
    entries = [fh]
    if ec > 1:
        exec "entries.extend(p4_pd.%s_get_next_entry_handles(fh, ec-1, sess_hdl=sess, dev_tgt=tgt))" % table
...
Optional parameters:
    ...
    #    try:
    #        exec "p4_pd.%s_update_hit_state(sess_hdl=sess, dev_id=tgt.dev_id)" % table
    #    except:
    ...
    try:
        exec "deh = p4_pd.%s_table_get_default_entry_handle(sess_hdl=sess, dev_tgt=tgt)" % table
        exec "entry = p4_pd.%s_get_entry(deh, read_from_hw, sess_hdl=sess, dev_id=tgt.dev_id)" % table
        print "*********************"
        print "DEFAULT ACTION: (Entry Handle:", deh, ")"
        print_action_desc(entry.action_desc)
    except:
        pass
...
contain a list of entries to delete (instead of all)
    ...
    cmd = "p4_pd.%s_table_delete(e, sess_hdl=sess, dev_id=tgt.dev_id)" % table
    for e in entries:
        exec cmd
    
    cmd = "p4_pd.%s_table_reset_default_entry(sess_hdl=sess, dev_tgt=tgt)" % table
    try:
        exec cmd
    except:
        pass
    cmd = "p4_pd.%s_idle_tmo_disable(sess_hdl=sess, dev_id=tgt.dev_id)" % table
    try:
        exec cmd
    except:
        pass
...
def get_action_profile_entries(table, count=None, read_from_hw=from_sw, sess_hdl=None, dev_tgt=None):
    (sess, tgt) = api_common_args(sess_hdl, dev_tgt)
    exec "ec = p4_pd.%s_get_act_prof_entry_count(sess_hdl=sess, dev_tgt=tgt)" % table

    if ec == 0:
        return []

    exec "fh = p4_pd.%s_get_first_member(sess_hdl=sess, dev_tgt=tgt)" % table
    entries = [fh]
    if ec > 1:
        exec "entries.extend(p4_pd.%s_get_next_members(fh, ec-1, sess_hdl=sess, dev_tgt=tgt))" % table
...
def dump_action_profile(table, entries=None, count=None, read_from_hw=from_sw, sess_hdl=None, dev_tgt=None):
    ...
    print "Action Profile", table, "Entry Count:", len(entries), "Total Occupancy:", ec
    
    for e in entries:
        exec "entry = p4_pd.%s_get_member(e, read_from_hw, sess_hdl=sess, dev_id=tgt.dev_id)" % table
        print "----------------------"
        print "Entry Handle:", e
...
def get_action_selector_groups(table, count=None, read_from_hw=from_sw, sess_hdl=None, dev_tgt=None):
    (sess, tgt) = api_common_args(sess_hdl, dev_tgt)
    exec "gc = p4_pd.%s_get_selector_group_count(sess_hdl=sess, dev_tgt=tgt)" % table
    if gc == 0:
        return []
    
    exec "fg = p4_pd.%s_get_first_group(sess_hdl=sess, dev_tgt=tgt)" % table
    groups = [fg]
    if gc > 1:
        exec "groups.extend(p4_pd.%s_get_next_groups(fg, gc-1, sess_hdl=sess, dev_tgt=tgt))" % table
...
def get_action_selector_group_members(table, group, count=None, read_from_hw=from_sw, sess_hdl=None, dev_tgt=None):
    (sess, tgt) = api_common_args(sess_hdl, dev_tgt)
    try:
        exec "fh = p4_pd.%s_get_first_group_member(group, sess_hdl=sess, dev_id=tgt.dev_id)" % table
        entries = [fh]
        ec=1
    except:
        return []
    ...
    while True:
        try:
            exec "more_entries = p4_pd.%s_get_next_group_members(group, fh, 120, sess_hdl=sess, dev_id=tgt.dev_id)" % table
...
def dump_action_selector_group(table, group, count=None,
    ...
    print "Selector Group: 0x%08x" % group, "   Entries:", entries
...
def dump(p4_object_name, entries=None, count=None, read_from_hw=from_sw, groups=None, entry_count=None, sess_hdl=None, dev_tgt=None):
    if TARGET=="bmv2":
        print """
PD APIs for BMv2-based targets do not yet implement the required functionality.
Please, use tofinobm_CLI
"""
        return
...
def clear_all(verbose=0, sess_hdl=None, dev_tgt=None):
    ...
    if TARGET=="bmv2":
        p4_pd.clean_all(sess_hdl=sess, dev_tgt=allpipes)
        return
